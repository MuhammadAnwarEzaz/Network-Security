...
def generate_message_to_send(aliceprkey,bobpkey):
    ...
    print('\nMessage concatenated with signature:',msg_plus_sign)
    #One time secret key
...
if Login:
    choice = input('Do you want to send the message (Enter Yes since I am sender): ')
    if choice == 'Yes':
        msg_to_send = generate_message_to_send(aliceprkey,bobpkey)
        client_socket.send(msg_to_send)
        msg = client_socket.recv(5000)
        if msg == b'You can exit':
            print('Exiting')
    else:
        msg = client_socket.recv(5000000)
        if msg == b'You can exit':
            print('Exiting')
        else:
            decrypt_message(msg,alicepr,bobpkey)
    client_socket.close()
else:
    print(" ")
client_socket.close()
